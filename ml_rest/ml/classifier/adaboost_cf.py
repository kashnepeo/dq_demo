...
class AdaBoostClass:
    ...
    def __init__(self):
        ...
        # 원본 데이터 로드
        data = pd.read_csv("./resource/classifier_sample.csv", sep=",", encoding="utf-8")
    ...
    def predict(self):
        ...
        # 스코어 확인
        print(f'Score = {accuracy_score(self._y_test, y_pred)}')
    ...
    def save_model(self, renew=False):
        ...
        if not renew:
            # 처음 저장
            joblib.dump(self._model, f'../model/{self._name}.pkl')
        else:
            # 기존 모델 대체
            if os.path.isfile(f'../model/{self._name}.pkl'):
                os.rename(f'../model/{self._name}.pkl', f'../model/{str(self._name) + str(time.time())}.pkl')
            joblib.dump(self._model, f'../model/{self._name}.pkl')
